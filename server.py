...
Roles:
  - Insurer A, Insurer B, Insurer C: each runs in their own browser, enters one number,
...
Protocol (pairwise OTP masking):
  ...
  insurer i and delivered to insurer j through /api/mask.
  Each insurer computes its masked share locally:
      s_A = x_A + r_AB + r_AC
      s_B = x_B - r_AB + r_BC
      s_C = x_C - r_AC - r_BC
...
Each round is gated by a 4-character alphanumeric session code, generated by
the server and displayed on the home page. Parties and the aggregator must
echo it back on every data API call; /api/reset rotates it.
"""
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/verify":
            ok = self._check_session(body.get("session", ""))
            if not ok:
                return self._reject_session()
