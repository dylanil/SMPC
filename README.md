...
- **Per-IP rate limits** (sliding window) on `/api/session/new` (10/min), `/api/join`/`/api/pubkey`/`/api/share`/`/api/reset` (30/min each). Caps memory-DoS via session-creation flooding, ECDSA-verify CPU exhaustion, and brute-force invite-token guessing. Read endpoints are unlimited.
