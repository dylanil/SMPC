...
Once the round completes, a **"Catch a dishonest aggregator"** card lets you see the verification story in action: it forges one share on a local copy of the result - keeping the reported sum self-consistent, the way a lying aggregator would - and re-runs the exact checks each participant's page performs. The arithmetic check passes on the forgery; the signature check catches it, because forging a share would need the participant's private signing key, which never left their browser. (The tampering is local-only; the server's session data is untouched, and tampered data can't land server-side anyway - signatures are verified at write time and first-write-wins locks each share.) The 3-party default is the smallest teaching example, chosen because the mask cancellation is easy to see; it is not a claim that tiny groups provide strong real-world privacy. Two honest caveats, which the page itself displays: a simulated round has no privacy (one tab necessarily knows every figure - that's why the page shows a reveal card at the end, something nobody can produce in a real round), and the simulation consumes all the session's invites, so create a fresh session for rounds with real participants.
