...
![A complete round of the real app, recorded live: a session is created, three simulated participants join and submit signed masked shares, the pairwise masks cancel to reveal only the average, the simulation-only reveal cross-checks it against the raw figures, and the tamper check catches a forged share.](docs/assets/demo-round.gif)

*A complete round, recorded from the real running app (generated by [`docs/assets/make_demo_gif.py`](docs/assets/make_demo_gif.py), never hand-captured): session creation, proof-of-work joins, signed masked shares, the masks cancelling into the average - then the tamper check catching a forged result. [Run it yourself](https://fl-wg-smpc.fly.dev/aggregator?demo=1).*
...
│       ├── qrcode.js      # Vendored MIT QR encoder (invite QR codes, generated locally)
